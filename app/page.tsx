...
export default function Home() {
  ...
  return <>
    ...
    <header className="app-header">
      <div className="brand"><img src={withBasePath("/icons/pcb-192-v3.png")} alt="PCB thermal conductivity calculator" width="54" height="54" /><div><h1>pcb-k計算</h1><p>THERMAL CALCULATOR</p></div></div>
    ...
    <Dialog open={installOpen} onOpenChange={setInstallOpen}><DialogContent className="app-dialog" showCloseButton={false}><DialogHeader><img className="install-icon" src={withBasePath("/icons/pcb-apple-v3.png")} width="76" height="76" alt="PCB 導熱計算桌面圖示" /><DialogTitle>將 pcb-k計算 加入主畫面</DialogTitle><DialogDescription>圖示為滿版 PCB 導熱圖，名稱為「pcb-k計算」。</DialogDescription></DialogHeader><div className="install-instructions"><h3>iPhone / iPad</h3><ol><li>用 <b>Safari</b> 開啟<a href={withBasePath("/install-v3/")}>新版圖示安裝頁</a>。</li><li>點「分享」，選「加入主畫面」。</li><li>確認圖示顯示 <b>PCB 導熱圖</b>，名稱為 <b>pcb-k計算</b>，開啟「作為網頁 App 打開」，再點「新增」。</li></ol><p>重開舊 App 不會更新主畫面圖示；請先加入新版，確認後再移除舊捷徑。</p><h3>Android / 電腦</h3><p>使用 Chrome 或 Edge，點網址列的安裝圖示，或選單中的「安裝應用程式」。</p><p className="install-offline">請先連網開啟，等頁尾顯示「已備妥離線使用」，之後即可離線計算。</p></div><DialogClose asChild><Button className="app-button">知道了</Button></DialogClose></DialogContent></Dialog>
