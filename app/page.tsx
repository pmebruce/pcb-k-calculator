...
export default function Home() {
  ...
  return <>
    ...
    <header className="app-header">
      <div className="brand"><img src={withBasePath("/icons/pcb-192-v2.png")} alt="PCB" width="54" height="54" /><div><h1>pcb-k計算</h1><p>THERMAL CALCULATOR</p></div></div>
    ...
    <Dialog open={installOpen} onOpenChange={setInstallOpen}><DialogContent className="app-dialog" showCloseButton={false}><DialogHeader><img className="install-icon" src={withBasePath("/icons/pcb-apple-v2.png")} width="76" height="76" alt="綠底白字 PCB 桌面圖示" /><DialogTitle>將 pcb-k計算 加入主畫面</DialogTitle><DialogDescription>圖示為完整「PCB」三個英文字母，名稱為「pcb-k計算」。</DialogDescription></DialogHeader><div className="install-instructions"><h3>iPhone / iPad</h3><ol><li>用 <b>Safari</b> 開啟<a href={withBasePath("/pcb-k/")}>新版安裝頁</a>。</li><li>點「分享」，選「加入主畫面」。</li><li>確認圖示顯示 <b>PCB</b>，名稱為 <b>pcb-k計算</b>，開啟「作為網頁 App 打開」，再點「新增」。</li></ol><p>舊主畫面的圖示與名稱需要重新加入才會更新；確認新版可用後，再移除舊捷徑。</p><p>若圖示仍只有一個字母，請更新至 iOS／iPadOS 26 或以上後再加入。</p><h3>Android / 電腦</h3><p>使用 Chrome 或 Edge，點網址列的安裝圖示，或選單中的「安裝應用程式」。</p><p className="install-offline">請先連網開啟，等頁尾顯示「已備妥離線使用」，之後即可離線計算。</p></div><DialogClose asChild><Button className="app-button">知道了</Button></DialogClose></DialogContent></Dialog>
